...
# Visualizar y guardar la precisión
plt.plot(history.history['accuracy'], label='Train Accuracy')
plt.plot(history.history['val_accuracy'], label='Validation Accuracy')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('../models/accuracy_plot.jpg')
plt.show()

# Guardar el historial en un archivo con pickle
with open('../models/history.pkl', 'wb') as file:
    pickle.dump(history.history, file)

# Guardar el modelo en el formato nativo de Keras (.keras)
model.save('../models/eye_detection_model.keras')

# Guardar el modelo en el formato HDF5 (.h5)
model.save('../models/eye_detection_model.h5')
